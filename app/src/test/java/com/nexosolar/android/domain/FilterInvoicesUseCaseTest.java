...
public class FilterInvoicesUseCaseTest {
    ...
    @Test
    public void invoke_whenFilterByStatus_returnsOnlyMatchingInvoices() {
        ...
        List<Invoice> result = useCase.invoke(baseList, statuses, null, null, null, null);
        ...
        assertEquals("La factura retornada debería tener estado Pagada",
                "Pagada", result.get(0).invoiceStatus);
    }

    @Test
    public void invoke_whenFilterByAmountRange_returnsInvoicesInRange() {
        // GIVEN: Lista de facturas con diferentes importes (100, 200, 300)

        // WHEN: Filtramos por rango de importe entre 150 y 250
        List<Invoice> result = useCase.invoke(baseList, null, null, null, 150.0, 250.0);
        ...
        assertEquals("La factura retornada debería tener importe de 200",
                200f, result.get(0).invoiceAmount, 0.01);
    }

    @Test
    public void invoke_whenNoFilters_returnsAllInvoices() {
        // GIVEN: Lista de facturas sin ningún filtro aplicado

        // WHEN: Ejecutamos sin filtros
        List<Invoice> result = useCase.invoke(baseList, null, null, null, null, null);
    ...
    @Test
    public void invoke_whenEmptyList_returnsEmptyList() {
        // GIVEN: Lista vacía de facturas
        List<Invoice> emptyList = new ArrayList<>();
        List<String> statuses = Collections.singletonList("Pagada");

        // WHEN: Filtramos una lista vacía
        List<Invoice> result = useCase.invoke(emptyList, statuses, null, null, null, null);
    ...
    @Test
    public void invoke_whenFilterByDate_returnsInvoicesInDateRange() {
        ...
        List<Invoice> result = useCase.invoke(baseList, null, startDate, endDate, null, null);
        ...
        assertEquals("La factura retornada debería ser la del 1 de febrero",
                LocalDate.of(2025, 2, 1), result.get(0).invoiceDate);
    }

    @Test
    public void invoke_whenMultipleFilters_returnsInvoicesMeetingAllCriteria() {
        ...
        List<Invoice> result = useCase.invoke(baseList, statuses, startDate, endDate, minAmount, maxAmount);
        ...
        assertEquals("Debería tener estado Pagada", "Pagada", invoice.invoiceStatus);
        assertEquals("Debería tener importe de 100", 100f, invoice.invoiceAmount, 0.01);
        assertEquals("Debería tener fecha de enero 2025",
                LocalDate.of(2025, 1, 1), invoice.invoiceDate);
    }

    @Test
    public void invoke_whenFilterByMultipleStatuses_returnsMatchingInvoices() {
        // GIVEN: Lista de facturas con diferentes estados
